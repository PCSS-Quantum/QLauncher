	...
	def __init__(
		...
		*,
		...
		if (not isinstance(sampler, BaseSamplerV1)) and (not isinstance(sampler, BaseSamplerV2)):
			raise ValueError(f'The sampler should be an instance of BaseSampler or BaseSamplerV2, but got {type(sampler)}')
...
class TrainQSVCKernel(Algorithm[TabularML, QiskitBackend]):
	...
	def __init__(self, kernel_circuit: QuantumCircuit, trainable_params: Sequence[Parameter] | None = None, **alg_kwargs) -> None:
	...
	def run(self, problem: TabularML, backend: QiskitBackend) -> Result:
		...
		trainable_kernel = TrainableFidelityQuantumKernel(
			feature_map=self.kernel, fidelity=ComputeUncomputeCustom(sampler=sampler), training_parameters=self.trainable
		)
